...
export async function search(this: IExecuteFunctions, index: number): Promise<INodeExecutionData[]> {
  const searchProperty = this.getNodeParameter('companySearchProperty', index) as string;
  const searchValue = this.getNodeParameter('companySearchValue', index) as string;
  ...
  if (returnAll) {
    const response = await userPilotApiRequestAllItems.call(this, 'GET', '/v1/companies/search', query);
    return response.map((item: any) => ({ json: item }));
  }

  const response = await userPilotApiRequest.call(this, 'GET', '/v1/companies/search', undefined, { ...query, limit });
