...
export async function getResponses(this: IExecuteFunctions, index: number): Promise<INodeExecutionData[]> {
  const surveyId = this.getNodeParameter('surveyId', index) as string;
  const returnAll = this.getNodeParameter('returnAll', index) as boolean;
  const limit = this.getNodeParameter('limit', index, 50) as number;

  if (returnAll) {
    const response = await userPilotApiRequestAllItems.call(this, 'GET', `/v1/nps/${surveyId}/responses`);
    return response.map((item: any) => ({ json: item }));
  }

  const response = await userPilotApiRequest.call(this, 'GET', `/v1/nps/${surveyId}/responses`, undefined, { limit });
