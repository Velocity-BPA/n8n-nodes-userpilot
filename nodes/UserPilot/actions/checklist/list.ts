...
export async function list(this: IExecuteFunctions, index: number): Promise<INodeExecutionData[]> {
  ...
  if (returnAll) {
    const response = await userPilotApiRequestAllItems.call(this, 'GET', '/v1/checklists');
    return response.map((item: any) => ({ json: item }));
  }

  const response = await userPilotApiRequest.call(this, 'GET', '/v1/checklists', undefined, { limit });
